...
def perc_bangalore_to_bangalore():
    bangalore_calls = 0
    internal_bangalore_calls = 0
    for call in calls:
        if call[0].startswith('(080)'):
            bangalore_calls += 1
            if call[1].startswith('(080)'):
                internal_bangalore_calls += 1
    return internal_bangalore_calls/float(bangalore_calls)*100


print("The numbers called by people in Bangalore have codes:")
print(*codes_called_by_bangalore(), sep='\n')
